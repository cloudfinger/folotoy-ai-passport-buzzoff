...
> verified `build/folotoy-ai-passport-buzzoff.bin` at offset `0x0` only when the
...
Firmware validation uses a fresh temporary build directory and an isolated `sdkconfig` generated from the tracked defaults. It does not consume or overwrite a developer's root `sdkconfig`, and it copies only the verified merged image to `build/folotoy-ai-passport-buzzoff.bin`. The gate also enforces the [protected Flash layout](protected-flash-layout.md): the protected `cardid` address, application size, partition-table MD5, and absence of device-specific identity data.
...
the validated `build/folotoy-ai-passport-buzzoff.bin` contains the complete checked
firmware layout.
