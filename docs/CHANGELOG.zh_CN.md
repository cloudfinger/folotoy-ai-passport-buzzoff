...
- 新增 Buzz Off 专属的根目录中英文 README，把 GitHub 首页由通用项目介绍改为 App 简介、操作方法、固件名称及完整五图展示。

- 补齐 Buzz Off 社区五图套装：已有封面加上开机、待机、捕蚊和蛙叫四张玩法图。额外图片均标为示意插画，不是实机截图；尚未加入待审核的社区稿件。

- 新增 Buzz Off 社区发布所用的原创 3:4 玩法封面，并明确标注为示意插画，而非实机截图。

- 将已验证、可刷入的 Buzz Off 完整固件统一命名为 `folotoy-ai-passport-buzzoff.bin`，并同步本地构建、CI、Release 与刷机说明。ESP-IDF 生成的应用单镜像保持原名。

- Buzz Off 待机及停止后立即隐藏蚊子和红色舌头。捕蚊动画与计数仅在 RUNNING 时推进；重新启动时从首帧播放。

- RUNNING 时每捕获两只蚊子，播放一段约一秒的 USGS 公有领域美洲牛蛙真实录音。叫声期间当前 PCM 底音会降低音量，codec 音量仅在叫声期间从 35 升至 55，结束后恢复 35；功能停止时叫声随即结束，待机旋律不变。新增主机测试和试听文件。

- 将 Buzz Off 原有的蓝天、草地、机器人和面板界面改为深色日系像素掌机布局。新增存放于 Flash 的 16 色湿地场景和青蛙闭嘴／张嘴动画、紧凑的电量和按键指示，并统一开机画面的配色。移除开机时的 I²C 地址遍历，仅在初始化完成后显示 `READY!`；声音和按键行为不变。

- 新增 Buzz Off 独立应用：像素青蛙吃蚊动画、三档 PCM 声音（12、16、20 kHz）、按键控制、捕食计数及主机测试。掌机风格的开机画面由上到下揭幕并逐字显示 CLOUDFINGER；原创 8-bit 背景旋律在开机和待机时播放，启动功能声时停止，停止功能声后恢复。实际声学输出与蚊虫反应仍需实机测量。
